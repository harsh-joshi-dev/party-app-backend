...
async def financial_summary(company_id: str):
    ...
    appointment_pipeline = [
        {"$match": {"company_id": company_id, "event_completed": {"$ne": "deleted"}}},
        {"$addFields": {
            "booking_amount": {"$toDouble": "$booking_amount"},
            "addon_total": {"$sum": {
                "$map": {"input": {"$ifNull": ["$tags", []]}, "as": "t", "in": {"$toDouble": {"$ifNull": ["$$t.price", 0]}}}
            }}
        }},
        {"$group": {
            "_id": {"year": {"$year": "$event_start_datetime"}, "month": {"$month": "$event_start_datetime"}},
    ...
    # ———— 2. Spents: salary and expense ————
    spent_pipeline = [
        {"$match": {"company_id": company_id}},
        {"$group": {
            "_id": {"year": {"$year": "$bought_date"}, "month": {"$month": "$bought_date"}},
    ...
    for s in spent_data:
        ...
        summary[key]["amount_spent_on_salary"] = s["amount_spent_on_salary"]
        summary[key]["total_expense"] = s["total_expense"]

    # Build final
    final = []
    tot_b, tot_a, tot_inc, tot_sal, tot_exp, tot_cake = [0]*6
